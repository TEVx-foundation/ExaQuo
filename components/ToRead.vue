<template>
    <v-card tile flat class="mx-0 mt-4">
        <v-card tile flat class="overflow-auto" color="transparent">

        <v-list-item class="mx-2 my-0 pr-0">
        ...
        <section v-else>
        <v-card dark flat color="transparent" v-for="(item, i) in fullList" :key="i" class="mx-2">
  ...
  <script>
    export default {
      ...
      methods: {
        async getEntireList() {
            ...
            try {
                ...
                .then((res) => {
                    ...
                    this.fullList = this.fullList.filter(el => !this.read.includes(el.id))
        ...
        async getNextList() {
            ...
            try {
                ...
                .then((res) => {
                    ...
                    this.nextList = this.nextList.filter(el => !this.read.includes(el.id))
        ...
        checkStatus(e) {
            if (this.cbooks.includes(e.id) === true) {
                return 1
            } else {
                return 0
            }
